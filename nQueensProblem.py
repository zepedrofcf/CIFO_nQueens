...
crossOverFunctions=["crossHalf","crossSinglePoint", "crossCycle", "crossPartialMatched", "crossGeometricSemantic"]
...
solveNQueens(150, 6, 7500, selectionFunctions[1], mutationFunctions[2], crossOverFunctions[0])
'''
...
n_values = range(4, 6)
populationSize = 200
selectionFunctions_=["Tournament Selection", "Roulette Wheel Selection"]
mutationFunctions_=["Position with conflict for random", "Individual For Random", "Shift Coordinate on Position with Conflict"]
crossOverFunctions_=["crossHalf","crossSinglePoint", "crossCycle", "crossPartialMatched", "crossGeometricSemantic"]
...
def Statistics(changing_variable, values, maxGenerations, selectionFunction, mutationFunction, crossOverFunction, n, populationSize, num_runs):
    ...
    for value in values:
        ...
        for _ in range(num_runs):
            if changing_variable == "populationSize":
                executionTime = solveNQueens(value, n, maxGenerations, selectionFunction, mutationFunction, crossOverFunction)
            elif changing_variable == "selectionFunction":
                executionTime = solveNQueens(populationSize, n, maxGenerations, value, mutationFunction, crossOverFunction)
            elif changing_variable == "mutationFunction":
                executionTime = solveNQueens(populationSize, n, maxGenerations, selectionFunction, value, crossOverFunction)
            elif changing_variable == "crossOverFunction":
                executionTime = solveNQueens(populationSize, n, maxGenerations, selectionFunction, mutationFunction, value)
            elif changing_variable == "n":
                executionTime = solveNQueens(populationSize, value, maxGenerations, selectionFunction, mutationFunction, crossOverFunction)
...
def plotResults(results, changing_variable):
    ...
    colors = [
        ...
        'lightgoldenrodyellow', 'lightgray', 'lightsalmon', 'lightsteelblue', 'lightbrown', 'lightmagenta']
...
maxGenerations = 7500
num_runs = 30
changing_variable = "selectionFunction"    # Change what you want to analyze
values = ["Tournament Selection", "Roulette Wheel Selection"]      # Values for the chosen variable
...
# Get statistics
results = Statistics(changing_variable, values, maxGenerations, selectionFunction, mutationFunction, crossOverFunction, n, populationSize, num_runs)
